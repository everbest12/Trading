...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('trading_sessions_sell.log')
    ]
)
...
# Place sell limit and sell stop orders
def place_orders(symbol):
    ...
    logging.info(f"Attempting to place sell orders for {symbol} at current price {price}")
    ...
    sell_limit = {
        ...
        "price": round(price + 0.0010, 5),  # Place sell limit above current price
        "sl": round(price + 0.0010 + sl_range, 5),  # Stop loss above entry
        "tp": round(price - 0.0015, 5),  # Take profit below entry
        "deviation": 10,
        "magic": 234567,  # Different magic number from buy bot
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
        "comment": "Sell limit by bot"
    }
    ...
    sell_stop = {
        ...
        "price": round(price - 0.0010, 5),  # Place sell stop below current price
        "sl": round(price - 0.0010 + sl_range, 5),  # Stop loss above entry
        "tp": round(price - 0.0015, 5),  # Take profit below entry
        "deviation": 10,
        "magic": 234567,
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
        "comment": "Sell stop by bot"
    }

    # Adjust order parameters for gold (XAUUSD and related pairs)
    if symbol.startswith("XAU"):
        for order in [sell_limit, sell_stop]:
            order["price"] = round(order["price"], 3)
            order["sl"] = round(order["sl"], 3)
            order["tp"] = round(order["tp"], 3)
...
# Main loop
def run_bot():
    logging.info("Starting sell trading bot...")
    initialize_mt5()
    try:
        while True:
            active_symbols = get_active_symbols()
            if active_symbols:
                logging.info("Starting sell order placement cycle...")
                for symbol in active_symbols:
                    place_orders(symbol)
                logging.info("Completed sell order placement cycle")
            time.sleep(60)
    except KeyboardInterrupt:
        logging.info("Sell bot stopped by user.")
